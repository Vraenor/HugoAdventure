...
void GameHub::goToGameScene(Ref *pSender) {

	auto scene = GameScene::createScene();
	Director::getInstance()->pushScene(TransitionFade::create(1.0, scene));
}

void GameHub::goToGameRoom(Ref *pSender) {
...
void GameHub::asteroidDone(Node *pSender) {
	pSender->stopAllActions();
	_asteroids.remove(pSender);
	removeChild(pSender);
}

void GameHub::spawnAsteroid(float dt) {
	Size visibleSize = Director::getInstance()->getVisibleSize();
...
void GameHub::onKeyPressed(EventKeyboard::KeyCode keyCode, Event *event){
	_pressedKey = keyCode;
	switch (_pressedKey) {
...
} void GameHub::onKeyReleased(EventKeyboard::KeyCode keyCode, Event *
	event){
...
void GameHub::onMouseMove(Event *event) {
...
void GameHub::setPhysicsWorld(PhysicsWorld *world) {
	mWorld = world;
	mWorld->setGravity(Vec2::ZERO);
}

bool GameHub::onContactBegin(PhysicsContact &contact) {
...
bool GameHub::init()
{
	...
	listener->onKeyPressed = CC_CALLBACK_2(GameHub::onKeyPressed, this);
	...
	contactListener->onContactBegin = CC_CALLBACK_1(GameHub::onContactBegin, this);
